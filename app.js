...
let $shownUsers = $divsShown.find('p.user');
...
$.getJSON('https://randomuser.me/api/?results=12&nat=us&inc=name,location,email,login,dob,cell,picture&noinfo').done(function(json){
	$.each(json.results, function(i,v){
		...
		$shownUsers[i].textContent = v.login.username;
		$shownEmails[i].textContent = v.email;
		$shownCities[i].textContent = `${city}, USA`;
		...
		$hidAddys[i].textContent = `${v.location.street}, ${city}, ${state}, USA ${v.location.postcode}`;
		$hidBdays[i].textContent = `Birthday: ${mo}/${day}/${yr}`;
